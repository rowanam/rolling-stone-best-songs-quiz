...
/**
 * This function starts the game. It calls the runGame function and passed it which game mode was chosen,
 * i.e. which game mode button has a "data-chosen" attribute of "true".
 */
function startGame() {
...
function runGame(gameMode) {
    ...
    function determineCorrectAnswer() {
        ...
        for others, the question will compare a feature of each of the songs to each other, 
        so the correct answer will be determined by which songs are compared. */
...
function generateCorrectAnswerDisplay(correctAnswerIndex, questionType, answerType) {
    ...
    // create a variable to store the span string of the correct song name
    let correctSongNameHTML = `<span class="correct-answer">${correctSongName}</span>`;

    // create the HTML content to display the correct answer
